module "tenant_authorizer_lambda" {
  ...
  lambda_source        = "${path.root}/../../code/TenantAuthorizer/bin/Release/net6.0/linux-x64/publish/TenantAuthorizer.zip"
  lambda_policy_list = {
  ...
  lambda_env_variables = {
    ...
    VALID_ISSUERS = "[\"https://dotnet6.blueprint.auth.com\"]"
    JWKS_STRING = <<EOF
{
  "keys": [
    {
...
EOF
  ...
  lambda_handler     = "TenantAuthorizer::TenantAuthorizer.ApiGatewayAuthorizer::FunctionHandler"
